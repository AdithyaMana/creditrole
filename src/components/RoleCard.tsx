...
export const RoleCard: React.FC<RoleCardProps> = ({ 
  ...
  const cardClasses = isMobile 
    ? `cursor-pointer ${isSelected ? 'border-blue-400 bg-blue-50 scale-105' : 'border-gray-200'}`
    : ` ${isDragOver ? 'border-blue-400 bg-blue-50 scale-105' : 'border-gray-200'}`;

  return (
    <div 
      className={`bg-white rounded-lg shadow-sm border p-4 transition-all duration-200 hover:shadow-md ${cardClasses}`}
